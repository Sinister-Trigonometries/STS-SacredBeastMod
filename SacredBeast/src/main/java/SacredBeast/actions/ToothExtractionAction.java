...
public class ToothExtractionAction extends AbstractGameAction {
    private int increaseGold;
    private DamageInfo info;
    private static final float DURATION = 0.1F;

    public ToothExtractionAction(AbstractCreature target, DamageInfo info) {
        this.info = info;
    ...
    public void update() {
        if (this.duration == 0.1F && this.target != null) {
            ...
            if (((this.target).isDying || this.target.currentHealth <= 0) && !this.target.halfDead && !this.target.hasPower("Minion")) {
                AbstractDungeon.player.obtainPotion(new CunningPotion());
